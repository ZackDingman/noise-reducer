// Traffic Max Voice Denoiser — complete app.js
...
const el = {
  ...
  trafficMax: document.getElementById('trafficMax'),
...
function lowShelf(fc, gainDB, sr){
  const A=Math.pow(10,gainDB/40), w0=2*Math.PI*fc/sr, cos=Math.cos(w0), sin=Math.sin(w0), alpha=sin/Math.SQRT2;
  const b0 = A*((A+1) - (A-1)*cos + 2*Math.sqrt(A)*alpha);
  const b1 = 2*A*((A-1) - (A+1)*cos);
  const b2 = A*((A+1) - (A-1)*cos - 2*Math.sqrt(A)*alpha);
  const a0 =      (A+1) + (A-1)*cos + 2*Math.sqrt(A)*alpha;
...
async function processChannel(x, opts, rn){
  ...
  // Filters
  const hpCut = opts.trafficMax ? 180 : (opts.traffic ? 120 : 80);
  const hp = onePoleHP(hpCut, TARGET_SR);

  const hums = [];
  if (opts.hum60){ [60,120,180,240,300,360,420,480].forEach(f=> hums.push(biquadNotch(f, 30, TARGET_SR))); }
  if (opts.hum50){ [50,100,150,200,250,300,350,400].forEach(f=> hums.push(biquadNotch(f, 30, TARGET_SR))); }

  const hissShelf = opts.hiss ? biquadHighShelf(8000, -10, TARGET_SR) : (v=>v);
  const baseLowShelf = opts.trafficMax ? lowShelf(300, -6, TARGET_SR)
                    :  opts.traffic   ? lowShelf(200, -4, TARGET_SR)
                                       : (v=>v);

  const passes = opts.trafficMax ? 4
               : opts.traffic    ? (opts.strength===2 ? 3 : 2)
                                  : (opts.strength===2 ? 2 : 1);
  const mix = clamp01(opts.wet);
  ...
  for(let i=0;i<frames;i++){
    ...
    const noSpeech = (vad < 0.5);
    const duckAmt = opts.trafficMax ? -15 : (opts.traffic ? -9 : 0);
    const duck = noSpeech ? Math.pow(10, duckAmt/20) : 1.0;
    const extraLowShelf = (opts.trafficMax && noSpeech) ? lowShelf(250, -12, TARGET_SR) : (v=>v);
...
async function processFile(file){
  try{
    ...
    const opts = {
      ...
      wet: (parseInt(el.wet.value,10)||100)/100,
      traffic: el.traffic.checked,
      trafficMax: el.trafficMax.checked,
